...
try:
    ...
    print "[Init] Failed to connect to NAOqi modules:", e
    tts = None
...
emotion_animations = {
    ...
    "confused": [
        "^start(animations/Stand/Gestures/SlowlyOfferBothHands_01)",
        "^start(animations/Stand/Gestures/GoToStance_Question_LeanFront)",
        "^start(animations/Stand/Gestures/BothArmsUpAndDown_HeadShake_01)"
    ],
    "disgust": [
    ...
    "fear": [
        "^start(animations/Stand/Gestures/StrongBothArmsUpAndDown_LeanLeft_01)",
        "^start(animations/Stand/Gestures/DoubleHorizontalBumpRightArm_01)"
    ]
}

# Fix missing emotion_to_animation dictionary by picking a random animation from emotion_animations for each emotion
emotion_to_animation = {}
for emotion, anim_list in emotion_animations.items():
    if anim_list:
        emotion_to_animation[emotion] = random.choice(anim_list)
    else:
        emotion_to_animation[emotion] = None
...
# === Run Animation with Speech ===
def speak_with_animation(emotion, sentence):
    tags = get_speech_tags(emotion)
    # Use old style string formatting compatible with Python 2.7
    speech = "\n\\vct=%(vct)d\\rspd=%(rspd)d\\vol=%(vol)d\\pau=%(pau)d\\ %(sentence)s" % {
        'vct': tags['vct'], 'rspd': tags['rspd'], 'vol': tags['vol'], 'pau': tags['pau'], 'sentence': sentence
    }

    animation = None
    if emotion.lower() in emotion_animations:
        animation = random.choice(emotion_animations[emotion.lower()])

    if animation and animation_player:
        anim_thread = threading.Thread(target=animation_player.run, args=(animation,))
        anim_thread.start()
...
# === Emotion Receiver with TTS & Animation ===
def emotion_receiver():
    ...
    while True:
        ...
        print "[Emotion Receiver] Connection from", addr
        try:
            ...
            try:
                # Python 2 bytes to unicode string
                data_str = data.decode('utf-8')
            except Exception:
                data_str = data  # fallback

            print "[Emotion Receiver] Received:", data_str
            emotion = data_str.strip().lower()

            # Select a random index for phrase from 0 to 9
            phrase_index = random.randint(0, 9)
            phrase_dict = emotion_to_phrase[phrase_index]
            phrase = phrase_dict.get(emotion, "I have no feelings about that.")

            speak_with_animation(emotion, phrase)
        except Exception, e:
